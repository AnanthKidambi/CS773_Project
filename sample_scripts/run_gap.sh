...
#     # --------------------------------------------------------------- #
#     # --------------------------------------------------------------- #
#     OUT_DIR=$STT_PATH/dopp_outputs_ip_stride/$gap_exe
#     mkdir -p $OUT_DIR
#     $STT_PATH/build/X86_MESI_Two_Level/gem5.opt --outdir=$OUT_DIR \
#     $CONFIG_FILE \
#     --num-cpus=1 --mem-size=4GB \
...
#     # --------------------------------------------------------------- #
#     # --------------------------------------------------------------- #
#     OUT_DIR=$STT_PATH/dopp_outputs_cplx_stride/$gap_exe
#     mkdir -p $OUT_DIR
#     $STT_PATH/build/X86_MESI_Two_Level/gem5.opt --outdir=$OUT_DIR \
#     $CONFIG_FILE \
#     --num-cpus=1 --mem-size=4GB \
...
for gap_exe in "${gap_executables[@]}"; do
    ...
    CONFIG_FILE=$STT_PATH/configs/example/se.py
    # --------------------------------------------------------------- #
    OUT_DIR=$STT_PATH/dopp_outputs_oracle/$gap_exe
    mkdir -p $OUT_DIR
    $STT_PATH/build/X86_MESI_Two_Level/gem5.opt --outdir=$OUT_DIR \
    $CONFIG_FILE \
    --num-cpus=1 --mem-size=4GB \
    ...
    # Wait for all background processes to finish before continuing
    echo "Started processing: $gap_exe"
done
wait 
